  ...
  if (result.title) {
    ...
    let definitions = result[0].meanings[0].definitions[0],
      phontetics = `${result[0].meanings[0].partOfSpeech}  ${
        result[0].phonetics[Object.keys(result[0].phonetics).length - 1].text
      }`;
    ...
    document.querySelector(".example span").innerText = `${
      definitions.example ? definitions.example : "No example found"
    }`;
    ...
    if (definitions.synonyms[0] == undefined) {
      synonyms.innerHTML = `No example found`;
    } else {
